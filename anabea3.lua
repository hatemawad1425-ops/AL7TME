...
local config = ac.configValues({
    ...
    showMapImg = true,
    teleportCooldown = 15,       -- ✅ مدة الانتظار بين كل انتقال (بالثواني)
    -- ✅ روابط مباشرة لكل صورة
    cursorPlayerImg = "https://i.imgur.com/kIXp7an.png",
    cursorChImg = "https://i.imgur.com/SCGtaeJ.png",
    cursorStdImg = "https://i.imgur.com/nZwb5tX.png",
    cursorNgImg = "https://i.imgur.com/lchOeYY.png",
    mapIconSpImg = "https://i.imgur.com/Sb74ZGi.png",
    mapIconPaImg = "https://i.imgur.com/znJv6cb.png",
    mapIconStImg = "https://i.imgur.com/DCCZp68.png",
})
...
-- ✅ تخزين روابط الصور من الـ config
local images = {
    cursorPlayer = config.cursorPlayerImg,
    cursorCh = config.cursorChImg,
    cursorStd = config.cursorStdImg,
    cursorNg = config.cursorNgImg,
    mapIconSp = config.mapIconSpImg,
    mapIconPa = config.mapIconPaImg,
    mapIconSt = config.mapIconStImg,
}

-- ✅ إضافة لوقو تيمنا من رابط خارجي
local teamnaLogoUrl = 'https://k.top4top.io/p_3672vv19h1.png'
...
-- ✅ متغير لتتبع آخر وقت انتقال
local lastTeleportTime = -999
...
local disabledCollisionEvent = ac.OnlineEvent({
    ...
    function(sender, data)
        ...
        if supportAPI_collision then
            ac.log(string.format('%s collision: [%d] %s', (data.disabled and 'Disabled own' or 'Enabled own'), sender.index, ac.getDriverName(sender.index)))
            physics.disableCarCollisions(sender.index, data.disabled)
            physics.disableCarCollisions(0, data.disabled)
        end
    end)

-- ✅ دالة للتحقق من إمكانية الانتقال
local function canTeleport()
    local currentTime = os.clock()
    local timeSinceLastTeleport = currentTime - lastTeleportTime
    return timeSinceLastTeleport >= config.teleportCooldown
end
...
local function getRemainingCooldown()
    local currentTime = os.clock()
    local timeSinceLastTeleport = currentTime - lastTeleportTime
    local remaining = config.teleportCooldown - timeSinceLastTeleport
...
local function window_FastTravelDebug()
    ...
    ui.text("Using Direct URLs for images")
    ui.separator()
    ...
    ui.tabBar('FastTravelDebugTabs', function()
        ...
        if ui.button("Copy Settings to Clipboard") then
            ...
            exportConfig[#exportConfig + 1] = "TeleportCooldown: " .. tostring(config.teleportCooldown)
...
function script.drawUI(dt)
    ...
    if mapMode then
        ...
        ui.transparentWindow('mapScreen', screenOffset, screenSize, function()
            ...
            -- ✅ عرض رسالة الانتظار إذا كان هناك كولداون
            if not canTeleport() then
                local remaining = getRemainingCooldown()
                local warningText = string.format('انتظر %.0f ثواني لانتقال مرة أخرى', remaining)
                local textSize = ui.measureDWriteText(warningText, 28)
                local textPos = vec2((screenSize.x - textSize.x) / 2, 50)
                
                ui.pushDWriteFont(fontBold)
                ui.beginOutline()
                ui.dwriteDrawText(warningText, 28, textPos, rgbm(1, 0.3, 0.3, 1))
                ui.endOutline(rgb.colors.black, 2)
                ui.popDWriteFont()
            end
            ...
            for i = 0, sim.carsCount - 1 do
                ...
                if carState.isActive and not ac.getCar(i).isHidingLabels then
                    ...
                    if 0 < screenPos.x and screenPos.x < 1 and 0 < screenPos.y and screenPos.y < 1 then
                        ...
                        ui.beginRotation()
                        ui.drawImage(images.cursorPlayer, screenPos - vec2(40, 40), screenPos + vec2(40, 40))
            ...
            for i, teleports in ipairs(onlineTeleports) do
                ...
                if 0 < screenPos.x and screenPos.x < 1 and 0 < screenPos.y and screenPos.y < 1 then
                    ...
                    local iconKey = 'mapIcon' .. pointType:sub(1,1):upper() .. pointType:sub(2)
                    if images[iconKey] then
                        ui.drawImage(images[iconKey], screenPos - vec2(40, 40), screenPos + vec2(40, 40))
                    end
                end
                ::continue::
            end

            if ui.isImageReady(teamnaLogoUrl) then
                local logoWidth = 400
                local logoHeight = 160
                local logoPos = vec2(10, 10)
                ui.drawImage(teamnaLogoUrl, logoPos, logoPos + vec2(logoWidth, logoHeight), rgbm(1, 1, 1, 0.9))
            end
            ...
            if hoverCID >= 0 or hoverMark >= 0 then
                ...
                ui.endOutline(rgb.colors.black, 1)
                ui.popDWriteFont()
                ui.drawImage(images.cursorCh, hoverCSP - vec2(40, 40), hoverCSP + vec2(40, 40))
            else
                -- ✅ تغيير الكيرسر بناءً على إمكانية الانتقال
                if teleportAvailable and canTeleport() then
                    ui.drawImage(images.cursorStd, mp - vec2(40, 40), mp + vec2(40, 40))
                else
                    ui.drawImage(images.cursorNg, mp - vec2(40, 40), mp + vec2(40, 40))
                end
            end
        end)
    elseif ac.getCar(0).speedKmh < 2 and sim.focusedCar == 0 and not ac.getUI().appsHidden then
        ...
        if ui.isImageReady(teamnaLogoUrl) then
            local logoWidth = 600
            local logoHeight = 240
            local logoPos = vec2(120, sim.windowHeight - 300)
            ui.drawImage(teamnaLogoUrl, logoPos, logoPos + vec2(logoWidth, logoHeight), rgbm(1, 1, 1, opacity * 0.85))
        end
        
        local textPos = vec2(300, sim.windowHeight - 190)
        ui.pushDWriteFont(fontBold)
        ui.beginOutline()
        ui.dwriteDrawText('M للانتقال السريع اضغط ', 24, textPos, rgbm(1, 1, 1, opacity))
        ui.endOutline(rgb.colors.black, 1.5)
        ui.popDWriteFont()
    end
end
...
function script.update(dt)
    ...
    if ac.getSim().isAdmin and debugOnlineExtra == nil then
        debugOnlineExtra = ui.registerOnlineExtra(ui.Icons.SettingsAlt, "FastTravelPlugin Debug", function() return true end, window_FastTravelDebug, function() debugWindowOpen = false end, ui.OnlineExtraFlags.Admin)
    end
end
...
function inputCheck()
    ...
    if mapMode and mapCamera then
        ...
        -- ✅ التحقق من إمكانية الانتقال قبل تنفيذه
        if teleportAvailable and canTeleport() and not debugWindowOpen then
